...
export function HairSalonProducts() {
  ...
  return (
    <section className="py-32 bg-[#FDFBF7] px-8 border-t border-[#1A1A1A]/10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          ...
          >
            <a href="#" className="text-[10px] font-sans tracking-[0.2em] uppercase text-[#1A1A1A] border-b border-[#1A1A1A] pb-1 hover:text-[#C4A484] hover:border-[#C4A484] transition-colors">
              Tümünü Keşfet
            </a>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {products.map((prod, i) => (
            <motion.div 
              ...
              className="group flex flex-col sm:flex-row bg-white border border-[#1A1A1A]/5 overflow-hidden hover:shadow-2xl transition-all duration-700"
            >
              ...
              <div className="w-full sm:w-1/2 p-10 flex flex-col justify-center bg-white relative">
                ...
                <h3 className="text-2xl font-serif text-[#1A1A1A] mb-3">{prod.name}</h3>
                <p className="font-sans font-light text-xs text-[#1A1A1A]/50 leading-relaxed mb-8">{prod.desc}</p>
                <div className="mt-auto flex items-center justify-between">
                  <span className="font-serif text-lg text-[#C4A484]">{prod.price}</span>
                  <button className="text-[9px] font-sans tracking-[0.2em] uppercase border border-[#1A1A1A] text-[#1A1A1A] px-6 py-2 hover:bg-[#1A1A1A] hover:text-[#FDFBF7] transition-colors">
                    Rezerve Et
                  </button>
                </div>
              </div>
