...
export function HairSalonTestimonials() {
  ...
  return (
    <section className="py-32 bg-[#FDFBF7] px-8 border-t border-[#1A1A1A]/10 overflow-hidden">
      <div className="max-w-4xl mx-auto text-center relative">
        
        {/* Giant Quote Icon Background */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[400px] font-serif text-[#1A1A1A]/5 italic leading-none pointer-events-none select-none">
          "
        </div>
        ...
        <div className="relative z-10 min-h-[300px] flex flex-col items-center justify-center">
          <AnimatePresence mode="wait">
            ...
            >
              <p className="text-2xl md:text-4xl font-serif text-[#1A1A1A] leading-relaxed italic mb-10 max-w-3xl">
                "{testimonials[current].text}"
              </p>
              
              <div className="flex flex-col items-center gap-2">
                <span className="text-[12px] font-sans tracking-[0.2em] uppercase text-[#1A1A1A] font-semibold">
          ...
          <div className="flex gap-4 mt-12">
            {testimonials.map((_, i) => (
              <button
                ...
                className={`transition-all duration-500 h-[1px] ${current === i ? 'w-10 bg-[#C4A484]' : 'w-4 bg-[#1A1A1A]/20 hover:bg-[#1A1A1A]/50'}`}
